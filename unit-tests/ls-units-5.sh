...
# 2. Sane registration
  # Register a dummy savegame for that gameserver
	TESTCOUNT="2.1"
        ARGS=(-X PUT --header 'Content-Type: application/json' --data '{"players":["maex", "ryan"], "gamename":"DummyGame1", "savegameid":"XYZ42"}')
        testMethod "$APIROOT/gameservices/DummyGame1/savegames/XYZ42?access_token=$ADMINTOKEN" "200"

  # Verify the total list of savegames now contains the test savegame
	TESTCOUNT="2.2"
	ARGS=(-X GET)
	testMethod "$APIROOT/gameservices/DummyGame1/savegames?access_token=$MAEXTOKEN" "200"
	assertexists XYZ42 $PAYLOAD

  # Verify the access of a non-existing savegame is now granted
	TESTCOUNT="2.3"
	ARGS=(-X GET)
	testMethod "$APIROOT/gameservices/DummyGame1/savegames/XYZ42?access_token=$MAEXTOKEN" "200"
	assertexists XYZ42 $PAYLOAD
	
  # Manually remove the registrated savegame again (rejected, wrong admin)
	TESTCOUNT="2.4"
	ARGS=(-X DELETE)
	testMethod "$APIROOT/gameservices/DummyGame1/savegames/XYZ42?access_token=$ADMINTOKEN5" "400"
...
# Illegal registrations (all reject)
  # Register a savegame as admin, but not the admin who is in charge of the game-service
	TESTCOUNT="3.1"
        ARGS=(-X PUT --header 'Content-Type: application/json' --data '{"players":["maex", "ryan"], "gamename":"DummyGame1", "savegameid":"XYZ42"}')
        testMethod "$APIROOT/gameservices/DummyGame1/savegames/XYZ42?access_token=$ADMINTOKEN5" "400"
  ...
  # Register a savegame as admin, but use a non existent game server
	TESTCOUNT="3.3"
        ARGS=(-X PUT --header 'Content-Type: application/json' --data '{"players":["maex", "ryan"], "gamename":"DummyGame1", "savegameid":"XYZ42"}')
        testMethod "$APIROOT/gameservices/DummyGame2/savegames/XYZ42?access_token=$ADMINTOKEN" "400"
...
# Sessions from savegames (ok)
  # Create a savegame that can be turned into a session
	TESTCOUNT="4.1"
        ARGS=(-X PUT --header 'Content-Type: application/json' --data '{"players":["maex", "ryan", "hyacinth"], "gamename":"DummyGame1", "savegameid":"XYZ42"}')
        testMethod "$APIROOT/gameservices/DummyGame1/savegames/XYZ42?access_token=$ADMINTOKEN" "200"


  # Create a session from a previously created savegame
	TESTCOUNT="4.2"
	ARGS=(-X POST --header 'Content-Type: application/json' --data '{"game":"DummyGame1", "creator":"maex", "savegame":"XYZ42"}')
	testMethod "$APIROOT/sessions?access_token=$MAEXTOKEN" "200"
  ...
  # Get session tokens for ryan, joerg, hyacinth, so the session can be joined
	TESTCOUNT="4.4"
        ARGS=(-X POST --user bgp-client-name:bgp-client-pw)
	...
	TESTCOUNT="4.6"
        testMethod "$TOKENROOT/token?grant_type=password&username=hyacinth&password=abc123" "200"
	USERTOKEN4=$(echo $PAYLOAD | cut -c 18-45)
	USERTOKEN4=$(escapetoken $USERTOKEN4)
  ...
  # Join the session (3rd join must be rejected)
	...
	TESTCOUNT="4.9"
        testMethod "$APIROOT/sessions/$SESSIONID/players/hyacinth?access_token=$USERTOKEN4" "400"
...
# Launching a loaded session
	...
	# Launch the session when there are not enough players (reject)
        TESTCOUNT="5.5"
        ARGS=(-X POST)
        testMethod "$APIROOT/sessions/$SESSIONID?access_token=$MAEXTOKEN" "400"	
	...
	# Launch the sessions (success)
        TESTCOUNT="5.7"
        ARGS=(-X POST)
        testMethod "$APIROOT/sessions/$SESSIONID?access_token=$MAEXTOKEN" "200"	
